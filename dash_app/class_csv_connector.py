...
class CsvConnection():
    ...
    def create_schema(self):
        ...
        self.cursor.execute('DROP TABLE IF EXISTS maindatabase.' + self.schema_name + '.' + self.table_name + ';')
        self.cursor.commit()
        self.cursor.execute('DROP SCHEMA IF EXISTS ' + self.schema_name + ';')
        self.cursor.commit()
        self.cursor.execute('CREATE SCHEMA ' + self.schema_name)
        self.cursor.commit()
    ...
    def csv_drop_schema(self):
        schema_name = self.schema_name
        self.cursor.execute('DROP SCHEMA IF EXISTS ' + schema_name + ';')
        self.cursor.execute('CREATE SCHEMA ' + schema_name)
        self.cursor.commit()

    def csv_alter_schema(self):
        schema_name = self.schema_name
        self.cursor.execute('ALTER SCHEMA ' + schema_name + ' TRANSFER dbo.' + self.table_name)
        self.cursor.commit()

    def drop_csv_tables(self):
        self.cursor.execute('DROP TABLE IF EXISTS maindatabase.'+self.schema_name+'.'+self.table_name+';')
        self.cursor.execute('DROP TABLE IF EXISTS maindatabase.dbo.'+self.table_name+';')
        self.cursor.commit()
